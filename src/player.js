...
// Due to some intense arrow functions, vim indention: "set cino=^1"
...
var MPRISPlayer = class MPRISPlayer {
  ...
  constructor(busName, owner) {
    ...
    new DBusIface.MediaServer2Player(
      busName,
      (proxy) => {
    ...
    new DBusIface.MediaServer2Playlists(
      busName,
      (proxy) => {
    ...
    new DBusIface.MediaServer2Tracklist(
      busName,
      (proxy) => {
    ...
    new DBusIface.RatingsExtension(
      busName,
      (proxy) => {
    ...
    this.connect(
      'update-player-state', 
      (player, state) => {
        //global.log("STATE IS: "+JSON.stringify(state));
  ...
  _init3() {
    if (Settings.MINOR_VERSION > 19) {
      // Versions before 3.20 don't have Mpris built-in. hideStockMpris setting
      this._signalsId.push(
        this._settings.connect(
          "changed::" + Settings.MEDIAPLAYER_HIDE_STOCK_MPRIS_KEY,
          (settings, key) => {
    ...
    this._signalsId.push(
      this._settings.connect(
        "changed::" + Settings.MEDIAPLAYER_VOLUME_KEY,
        (settings, key) => {
    ...
    // showPosition settingn
    this._signalsId.push(
      this._settings.connect(
        "changed::" + Settings.MEDIAPLAYER_POSITION_KEY,
        (settings, key) => {
    ...
    this._signalsId.push(
      this._settings.connect(
        "changed::" + Settings.MEDIAPLAYER_RATING_KEY,
        (settings, key) => {
    ...
    this._signalsId.push(
      this._settings.connect(
        "changed::" + Settings.MEDIAPLAYER_PLAY_STATUS_ICON_KEY,
        (settings, key) => {
    ...
    this._signalsId.push(
      this._settings.connect(
        "changed::" + Settings.MEDIAPLAYER_TRACKLIST_RATING_KEY,
        (settings, key) => {
    ...
    this._signalsId.push(
      this._settings.connect(
        "changed::" + Settings.MEDIAPLAYER_STOP_BUTTON_KEY,
        (settings, key) => {
    ...
    this._signalsId.push(
      this._settings.connect(
        "changed::" + Settings.MEDIAPLAYER_LOOP_STATUS_KEY,
        (settings, key) => {
    ...
    this._signalsId.push(
      this._settings.connect(
        "changed::" + Settings.MEDIAPLAYER_BUTTON_ICON_STYLE_KEY,
        (settings, key) => {
    ...
    this._signalsId.push(
      this._settings.connect(
        "changed::" + Settings.MEDIAPLAYER_PLAYLISTS_KEY,
        (settings, key) => {
    ...
    this._signalsId.push(
      this._settings.connect(
        "changed::" + Settings.MEDIAPLAYER_PLAYLIST_TITLE_KEY,
        (settings, key) => {
    ...
    this._signalsId.push(
      this._settings.connect(
        "changed::" + Settings.MEDIAPLAYER_TRACKLIST_KEY,
        (settings, key) => {
    ...
    this._tracklistSignalsId.push(
      this._mediaServerTracklist.connectSignal(
        'TrackListReplaced',
        (proxy, sender, [trackIds, currentTrackId]) => {
    ...
    this._tracklistSignalsId.push(
      this._mediaServerTracklist.connectSignal(
        'TrackAdded',
        (proxy, sender, [trackMetadata, afterTrackId]) => {
          let insertIndex = -1;
    ...
    this._tracklistSignalsId.push(
      this._mediaServerTracklist.connectSignal(
        ...
        (proxy, sender, [trackId, updatedMetadata]) => {
          this.emit('update-player-state', new PlayerState({updatedMetadata: updatedMetadata}));
        }
      )
    );

    this._playlistsId = this._mediaServerPlaylists.connectSignal(
      'PlaylistChanged',
      (proxy, sender, [updatedPlaylist]) => {
    ...
    this._seekedId = this._mediaServerPlayer.connectSignal(
      'Seeked',
      (proxy, sender, [value]) => {
        if (value > 0) {
  ...
  populate() {
    ...
    if (newState.playlistCount > 0 && newState.playlistTitle) {
      this._playlistTimeOutId = Mainloop.timeout_add_seconds(
        1,
        () => {
          this._playlistTimeOutId = 0;
          this._getPlaylists(this.state.orderings);
    ...
    if (newState.hasTrackList && !isDummyTracklist) {
      this._tracklistTimeOutId = Mainloop.timeout_add_seconds(
        1,
        () => {
          this._tracklistTimeOutId = 0;
          this._getTracklist();
  ...
  _getPlaylists(orderings) {
    ...
    this._mediaServerPlaylists.GetPlaylistsRemote(
      0, 
      100, 
      ordering, 
      false,
  ...
  _getTracklist() {
    ...
    if (this._trackIds.length === 0) {
      ...
      this._mediaServerTracklist.GetTracksMetadataRemote(
        this._trackIds,
        ([trackListMetaData]) => {
  ...
  _startTimer() {
    if (this._timerId === 0) {
      this._timerId = Mainloop.timeout_add_seconds(
        1,
        () => {

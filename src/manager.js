...
var PlayerManager = class PlayerManager {
    ...
    this._ownerChangedId = this._dbus.connectSignal('NameOwnerChanged', (proxy, sender, [name, old_owner, new_owner]) => {
      if (name_regex.test(name)) {
        if (!this._disabling) {
          ...
          var propValue;
          for(var propName in this._players) {
            propValue = this._players[propName]
          }
  ...
  getPlayersByStatus(status, preference) {
    ...
    return Object.keys(this._players).map((owner) => {
...
let ui = new UI.PlayerUI(player);
        ...
        let playerUpdateId = playerItem.player.connect('player-update', this._onPlayerUpdate.bind(this));
